...
async function main() {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (args[i] === "--image") (opt.images ||= []).push(args[++i]);
  ...
  if (opt.images?.length || opt.image || opt.imageDir) {
    // ---------- 本地图片模式 ----------
    const imgs = opt.images?.length ? opt.images : (() => {
      const img = findLocalImage(opt.image, opt.imageDir);
      return img ? [img] : [];
    })();
    if (!imgs.length) throw new Error("No local image found (Downloads/Desktop/ShareX). Use --image <path> to specify.");
    console.log("IMAGE:", imgs.join(" | "));
    stamp("使用本地图片");
