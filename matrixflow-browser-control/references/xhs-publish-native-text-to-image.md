...
`--schedule "YYYY-MM-DD HH:mm"`（也支持 `明天 20:00`、`后天 20:00`、`20:00`）：

1. 自动打开「更多设置 → 定时发布」开关（点右侧 `.d-switch` 滑块，点文字无效）；
2. 打开日期时间面板；目标日期非今天则先选日期（点日期后面板自动关闭，需重开）；
3. 选小时（左列）+ 分钟（右列）：分钟列在需要滚动时平台虚拟滚动会导致点不准，
   脚本用「滚到容器内 + elementFromPoint 命中验证」尽力点，点不准时保留平台默认
   （当前时间 +30 分钟）并打印警告；
4. 验证显示值包含目标日期，然后正常发布，笔记管理显示「定时发布 HH:mm」。

已知限制：精确到任意分钟的定时在平台分钟选择器需滚动时不稳定（实测相邻默认分钟 ±2 可点准），
需要精确分钟时发布后在发布页人工微调分钟即可，不影响定时本身。
