...
## 🚀 当前推荐用法（MatrixFlow 1.68+，无需手动装 Codex）

从 2026-08 起，MatrixFlow 客户端已内置 **AI 智能助手**，客户不再需要手动装 Codex / 配 DeepSeek / 装技能：

1. 安装并登录 [MatrixFlow](https://browser.lingjingxia.com)（检查更新到 1.68+）；
2. 打开左侧「AI 助手」：把 **DeepSeek Key（必填）** 和 **阿里云百炼 Key（建议，看图/过验证码）** 直接发在对话里，AI 自动保存配置；
3. Codex 会自动后台安装（约 170MB，来自官方下载服务器），技能（本仓库）会自动同步到本机；
4. 直接发指令即可：养号打标签、发笔记、评论截流、创建窗口、跑 RPA 任务。

> 本仓库的 `matrixflow-browser-control` 技能仍会随客户端自动同步更新，也是 Codex 引擎执行站点级流程（小红书注册、发笔记、Facebook 发帖等）的核心脚本库。
> 以下为手动安装与命令参考，适合想直接用 Codex 命令行或纯脚本的用户。
