...
> ⚡ **推荐（MatrixFlow 1.68+）**：安装并登录 MatrixFlow 后，打开左侧「AI 助手」，把 DeepSeek Key（必填）和阿里云百炼 Key（建议）直接发在对话里，Codex、技能、视觉模型都会自动安装配置，无需手动操作。下面这份手动教程保留给想用 Codex 命令行或纯脚本的用户参考。
